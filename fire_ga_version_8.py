...
class env():
    def __init__(self):
        ...
        self.action_dim = len(self.fleet_dict)*len(self.fire_coordinates)*self.one_missile_fly_max_time*self.total_missiles
        self.state = [1]*self.state_dim+[time for fleet in self.fleet_dict.values() for time in fleet.fire_channels]
        self.done = False
        self.done_state = [0 for i in range(len(self.fleet_dict))]
    def reset(self):
        self.state = [1]*self.state_dim+[0 for fleet in self.fleet_dict.values() for time in fleet.fire_channels]
        self.done = False
    ...
    def step(self,action):
        ...
        #     print("distance_a_target:{}".format(distance_a_target))
            ...
            if distance_a_target>fleet_dict[missile_a.target].distance :
            ...
            if fleet_dict[missile_a.target].channel_can_fire_count()>=N_0:
                fleet_dict[missile_a.target].use_channel(N_0)
            else:
                fleet_dict[missile_a.target].use_channel(fleet_dict[missile_a.target].channel_can_fire_count())
                N_0_residual=N_0-fleet_dict[missile_a.target].channel_can_fire_count()
