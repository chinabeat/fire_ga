...
class env():
    def __init__(self):
        ...
        #               "j": self.fleet_j
        #               }
        self.coordinates = [[x, y] for x in range(-20, 61, 20) for y in range(-20, 41, 20)]
    ...
    def step(self,action):
        ...
        for fleet in fleet_dict.keys():
            fleet_dict[fleet].freeze_time = 0
        # N_0 = 3
        P1 = 0.98
        P_1_list = list()
        for N_1 in range(N_0 + 1):
        ...
        #     print("distance_a_target:{}".format(distance_a_target))
            ...
            if distance_a_target>fleet_dict[missile_a.target].distance or fleet_dict[missile_a.target].freeze_time>0:
            ...
            for fleet in fleet_dict.keys():
                if (fleet_dict[fleet].dead==False) and \
                        (fleet_dict[fleet].distance_to_missile(missile_a) < fleet_dict[fleet].distance and
                                 fleet_dict[fleet].freeze_time==0):
