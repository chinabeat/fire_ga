...
class DQN():
    ...
    def perceive(self,episode,step, state, action, reward, next_state, done):
    ...
    def train_Q_network(self,episode,step):
        ...
        print("episode:{},step:{},cost:{}".format(episode,step,self.cost.eval(feed_dict={
            self.y_input: y_batch,
            self.action_input: action_batch,
            self.state_input: state_batch
        })))
        if self.cost.eval(feed_dict={self.y_input: y_batch,self.action_input: action_batch,self.state_input: state_batch}) <0.00001:
            return False
    ...
    def save(self):
        ...
        self.save_path = self.saver.save(self.session, './model_version_9_1/model_ckpt')
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        for step in range(STEP):
            ...
            perceive_state = agent.perceive(episode,step,state, action, reward, next_state, done)
            state = next_state
            if done or (env.total_missiles <= 0 and len(env.flying_missiles_dict) ==0):
                break
            if perceive_state == False:
                break
        if perceive_state == False:
            break
        ...
        if episode % 100 == 0:
            ...
            for i in range(TEST):
                ...
                for j in range(STEP):
                    ...
                    print("test episode: {}, step: {},state: {}, reward: {}, done: {}".format(i, j,state, reward, done))
                    total_reward += reward
